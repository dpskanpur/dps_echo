...
export function RbacMatrixTable({
  ...
  return (
    <div className="space-y-6 w-full max-w-full">
      ...
      {/* Top Controls: Search, Stat Badge & Add User Button */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-2 text-xs font-semibold text-slate-500 bg-slate-100 px-3.5 py-2 rounded-xl border border-slate-200">
            <span>Total Staff: <strong className="text-slate-900">{users.length}</strong></span>
          </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-900 text-white text-[11px] font-bold uppercase tracking-wider border-b border-slate-800">
              <th className="p-3.5 min-w-[190px]">Staff Member</th>
              <th className="p-3.5 min-w-[150px]">School Scope</th>
              <th className="p-3.5 min-w-[140px]">Status & Preset</th>
              {APP_MODULES.map((mod) => (
                <th
                  key={mod.id}
                  className="p-2.5 text-center min-w-[95px] border-l border-slate-800"
                >
                  <div className="truncate font-black">{mod.label.split(" ")[0]}</div>
                  <div className="text-[9px] font-normal text-slate-400 normal-case flex items-center justify-center gap-2 mt-1 font-mono tracking-tight">
                    <span title="View Permission (Read)">V</span>
                    <span title="Update Permission (Write)">U</span>
                    <span title="Delete Permission (Delete)">D</span>
                  </div>
                </th>
              ))}
              <th className="p-3.5 text-right min-w-[70px] border-l border-slate-800">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {filteredUsers.length === 0 ? (
              <tr>
                <td colSpan={APP_MODULES.length + 4} className="p-12 text-center text-slate-400">
              ...
              filteredUsers.map((u) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-3.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center font-black text-slate-700 text-xs shrink-0 shadow-2xs">
                          {u.avatarUrl ? (
                    ...
                    {/* 2. School Scope Dropdown */}
                    <td className="p-3.5">
                      <select
                        value={u.campusId || "ALL"}
                        disabled={isSuperAdmin}
                        onChange={(e) => handleCampusChange(u.id, e.target.value)}
                        className="w-full text-[11px] bg-emerald-50/80 border border-emerald-200/90 rounded-lg px-2 py-1 text-emerald-950 font-bold hover:bg-emerald-100 focus:outline-none focus:ring-1 focus:ring-[#0F9D58] cursor-pointer disabled:opacity-50 transition shadow-2xs"
                    ...
                    <td className="p-3.5">
                      <div className="space-y-1">
                        <div>
                          ...
                          >
                            {isSuspended ? "Suspended" : u.status === "PENDING" ? "Pending" : "Active"}
                          </span>
                        </div>
                        ...
                        {!isSuperAdmin ? (
                          <select
                            ...
                            className="w-full text-[10px] bg-slate-50 border border-slate-200 rounded-lg px-1.5 py-0.5 text-slate-700 font-medium hover:bg-slate-100 focus:outline-none focus:ring-1 focus:ring-[#0F9D58] cursor-pointer"
                          >
                            <option value="" disabled>
                              Preset...
                            </option>
                            <option value="FULL_ADMIN">⭐ Full Admin</option>
                            <option value="FEES_SPECIALIST">💳 Fee Desk</option>
                            <option value="ADMISSIONS_SPECIALIST">🎓 Admissions</option>
                            <option value="VIEW_ALL">👁️ View Only</option>
                    ...
                    {/* 4–9. Module Permissions Columns */}
                    {APP_MODULES.map((mod) => {
                      ...
                      return (
                        <td
                          key={mod.id}
                          className="p-2 text-center border-l border-slate-100 bg-slate-50/20"
                        >
                          <div className="flex items-center justify-center gap-2">
                            ...
                            >
                              <input
                                ...
                                className="w-3.5 h-3.5 rounded text-[#0F9D58] focus:ring-[#0F9D58] border-slate-300 accent-[#0F9D58] cursor-pointer"
                              />
                            </label>
                            ...
                            >
                              <input
                                ...
                                className="w-3.5 h-3.5 rounded text-blue-600 focus:ring-blue-600 border-slate-300 accent-blue-600 cursor-pointer"
                              />
                            </label>
                            ...
                            >
                              <input
                                ...
                                className="w-3.5 h-3.5 rounded text-rose-600 focus:ring-rose-600 border-slate-300 accent-rose-600 cursor-pointer"
                              />
                            </label>
                          </div>
                        </td>
